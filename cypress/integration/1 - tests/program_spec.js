...
describe('Error toleration', () => {
  ...
  it ("Javascript injection 1 - pop up message not to be displayed", () => {
    cy.contains('Add a New Program').click()
  ...
  it ("Javascript injection 2 - things not to move around on the web page", () => {
    cy.contains('Add a New Program').click()
    ...
    cy.contains('.card', "javascript:document.body.contentEditable='true';document.designMode='on';void 0") 
    .children('.card-footer')
    .children('button.btn.btn-danger')
    .click()
    cy.contains('Yes').click()
  ...
  it ("Javascript injection 3 - pictures not to fly around", () => {
    cy.contains('Add a New Program').click()
    cy.get('input[name=name]').type('javascript:R=0; x1=.1; y1=.05; x2=.25; y2=.24; x3=1.6; y3=.24; x4=300; y4=200; x5=300; y5=200; DI=document.images; DIL=DI.length; function A(){for(i=0; i-DIL; i++){DIS=DI[ i ].style; DIS.position="absolute"; DIS.left=Math.sin(R*x1+i*x2+x3)*x4+x5; DIS.top=Math.cos(R*y1+i*y2+y3)*y4+y5}R++}setInterval("A()",5); void(0);', { parseSpecialCharSequences: false })
    cy.get('input[name=duration_days]').type('180')
    cy.get('textarea[name=description]').type('javascript:alert(document.forms[0].to.value=gsd34543)')
    cy.get('input[type=file]').selectFile('cypress/fixtures/LHLpic1.png')
    cy.contains('Submit').click()
    cy.contains('Welcome to javascript:R=0; x1=.1; y1=.05; x2=.25; y2=.24; x3=1.6; y3=.24; x4=300; y4=200; x5=300; y5=200; DI=document.images; DIL=DI.length; function A(){for(i=0; i-DIL; i++){DIS=DI[ i').should('be.visible')
    cy.contains('View All Programs').click()
    cy.contains('.card','javascript:R=0; x1=.1; y1=.05; x2=.25; y2=.24; x3=1.6; y3=.24; x4=300; y4=200; x5=300; y5=200; DI=document.images; DIL=DI.length; function A(){for(i=0; i-DIL; i++){DIS=DI[ i').should('exist')
...
it ("Javascript injection 4 - not to see the actual url", () => {
  Cypress.config('defaultCommandTimeout', 10000);
  cy.contains('Add a New Program').click()
  cy.get('input[name=name]').type('javascript:alert("The actual url is:\t\t" + location.protocol + "//" + location.hostname + "/" + "\nThe address URL is:\t\t" + location.href + "\n" + "\nIf the server names do not match, this may be a spoof.");')
  cy.get('input[name=duration_days]').type('180')
  cy.get('textarea[name=description]').type('javascript:alert(document.forms[0].to.value=gsd34543)')
  cy.get('input[type=file]').selectFile('cypress/fixtures/LHLpic1.png')
  cy.contains('Submit').click()
  cy.contains('Welcome to javascript:alert("The actual url is:').should('be.visible')
  cy.contains('View All Programs').click()
  cy.contains('.card','javascript:alert("The actual url is:').should('be.visible')
  cy.contains('.card','javascript:alert("The actual url is:') 
  .children('.card-footer')
  .children('button.btn.btn-danger')
  .click()
  cy.contains('Yes').click()
